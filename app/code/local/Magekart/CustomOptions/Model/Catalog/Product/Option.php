...
class Magekart_CustomOptions_Model_Catalog_Product_Option extends Mage_Catalog_Model_Product_Option {

    const OPTION_TYPE_SWATCH = 'swatch';
    const OPTION_TYPE_MULTISWATCH = 'multiswatch';

    const OPTION_TYPE_SIZE = 'size';
    const OPTION_TYPE_NUMBER_OF_PAGES = 'number_of_pages';
    const OPTION_TYPE_QUANTITY_BREAKS = 'quantity_breaks';
    const OPTION_TYPE_QUANTITY_RANGES = 'quantity_ranges';
    const OPTION_TYPE_INK_COLOR = 'ink_color';
    const OPTION_TYPE_MISC = 'misc';
    const OPTION_TYPE_PAPERSTOCK = 'paperstock';
    const OPTION_TYPE_PROOF_OPTIONS = 'proof_options';
    const OPTION_TYPE_FINISHING = 'finishing';
    const OPTION_TYPE_TURNAROUND = 'turnaround';
    const OPTION_TYPE_FILE_UPLOAD = 'file_upload';
    const OPTION_GROUP_PRINTED = 'printed';
...
//                exit;
                ...
                foreach ($productIds as $productId) {                
                    ...
                    foreach ($options as $option) {
                        ...
                        // insert option values
                        if (($this->getGroupByType($option['type'])==self::OPTION_GROUP_SELECT || $this->getGroupByType($option['type'])==self::OPTION_GROUP_PRINTED) && count($option['values'])>0) {
                            foreach ($option['values'] as $value) {
                                if (isset($value['is_delete']) && $value['is_delete'] == 1) continue;
                    ...
                    foreach ($allStoreOptions as $storeOptions) {
                        foreach ($storeOptions['hash_options'] as $option) {                            
                            ...
                            // insert option values
                            if ($optionId && ($this->getGroupByType($option['type'])==self::OPTION_GROUP_SELECT || $this->getGroupByType($option['type'])==self::OPTION_GROUP_PRINTED) && count($option['values'])>0) {
                                foreach ($option['values'] as $value) {
    ...
    public function saveOptionValue($optionId, $value, $prevValue, $newValue, $optionTypeId = 0, $storeId = 0) {
        ...
        if ($storeId==0) {
            ...
            if (isset($value['weight'])) $optionValue['weight'] = $value['weight']; else  $optionValue['weight'] = 0;

            //remove not needed fields
            $dataForSaving = $value;
            unset($dataForSaving['option_type_id'],$dataForSaving['is_delete'],$dataForSaving['in_group_id']);
            $optionValue['serialised_values'] = @serialize($dataForSaving);
    ...
    public function saveOptions() {
        ...
        foreach ($options as $key => $option) {
            ...
            if (!isset($option['use_custom_title'])) $options[$key]['use_custom_title'] = 0;
        }
        $this->setOptions($options);        
        ...
        if (is_array($this->getData('values'))) {
            ...
            foreach ($this->getData('values') as $key => $value) {
                if (isset($value['option_type_id'])) {
                    ...
                    $value['sku'] = isset($value['sku']) ? trim($value['sku']):'';
                    ...
                    $optionValue = array(
                        'option_id' => $optionId,
                        'sku' => $value['sku'],
                        'sort_order' => isset($value['sort_order'])?$value['sort_order']:1,
                        'customoptions_qty' => $customoptionsQty,
                        'default' => array_search($key, $defaultArray) !== false ? 1 : 0,
                        'in_group_id' => $value['in_group_id'],
                        'serialised_values' => @serialize($dataForSaving),
                    );                
                    $value['option'] = $this;
                    $value['title'] = Mage::helper('brocustomoption')->getValueTitle($value);
                    if (isset($value['dependent_ids'])) $optionValue['dependent_ids'] = $value['dependent_ids'];
                    if (isset($value['weight'])) $optionValue['weight'] = $value['weight']; else $optionValue['weight'] = 0;
    ...
    public function getGroupByType($type = null) {
        ...
        $optionGroupsToTypes = array(
            ...
            self::OPTION_TYPE_SIZE => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_NUMBER_OF_PAGES => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_QUANTITY_BREAKS => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_QUANTITY_RANGES => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_INK_COLOR => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_MISC => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_PAPERSTOCK => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_PROOF_OPTIONS => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_FINISHING => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_TURNAROUND => self::OPTION_GROUP_PRINTED,
            self::OPTION_TYPE_FILE_UPLOAD => self::OPTION_GROUP_PRINTED,
    ...
    public function groupFactory($type)
    {
        ...
        if (!empty($group)) {
            //we will have separate model for each type of Printed product
            switch($group){
                case 'printed':
                    $typeModel = uc_words($type, '');
                    return Mage::getModel('brocustomoption/product_option_type_' . $typeModel);
                    break;
                default:
