...
class Magekart_CustomOptions_Model_Observer {
        ...
        if ($options) {            
            ...
            foreach ($options as $optionId => $option) {                     
                ...
                if ($productOption->getGroupByType($optionType)!=Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT && 
                        $productOption->getGroupByType($optionType) != 'printed') 
                    continue;
                if (!is_array($option)) $option = array($option);
                //product Qty
                ...
                foreach ($option as $optionTypeId) {
                    if (!$optionTypeId) continue;
                    $row = $productOption->getOptionValue($optionTypeId);
                    if ((isset($row['weight'])) || $optionType === 'quantity_ranges') {
                        ...
                        switch ($optionType) {
                            ...
                            case 'quantity_breaks':    
                                $data = @unserialize($row['serialised_values']);    
                                if (isset($post['options_'.$optionId.'_qty'])) $optionQty = intval($post['options_'.$optionId.'_qty']); else $optionQty = 1;
                                $optQty = $data['qty'];
                                break;
                            case 'quantity_ranges':
                                $row['weight'] = $productOption->getWeightPerOne();
                                $optionQty = $optionTypeId;
                                $optQty = $optionQty;
                                 break;  
                            case 'multiple':
                                $optionQty = 1;                            
                        ...
                        $data = @unserialize($row['serialised_values']);
                        if(isset($data['weight_curve'])){
                            if($productOption->getData('price_curve_type') == 2){
                               $weight = isset($data['weight_curve'][$optQty])?$data['weight_curve'][$optQty] : $weight;
                            } elseif($productOption->getData('price_curve_type') == 3){
                               foreach($data['weight_curve'] as $key=>$val){
