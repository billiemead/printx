...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('customoptions/option_type_tier_price')}` (
  ...
  CONSTRAINT `FK_MAGEKART_CUSTOM_OPTIONS_INDEX_OPTION_TYPE_TIER_PRICE` FOREIGN KEY (`option_type_price_id`) REFERENCES `{$installer->getTable('catalog/product_option_type_price')}` (`option_type_price_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('customoptions/option_type_image')}` (
  ...
  CONSTRAINT `FK_MAGEKART_CUSTOM_OPTIONS_INDEX_OPTION_TYPE_IMAGE` FOREIGN KEY (`option_type_id`) REFERENCES `{$installer->getTable('catalog/product_option_type_value')}` (`option_type_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('customoptions/option_view_mode')}` (
  ...
  CONSTRAINT `FK_MAGEKART_CUSTOM_OPTIONS_VIEW_MODE_OPTION` FOREIGN KEY (`option_id`) REFERENCES `{$installer->getTable('catalog/product_option')}` (`option_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_MAGEKART_CUSTOM_OPTIONS_VIEW_MODE_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core/store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;
");
