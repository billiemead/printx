...
class Magekart_Adminhtml_Block_Customoptions_Options_Edit_Tab_Options_Option extends Magekart_Adminhtml_Block_Customoptions_Adminhtml_Catalog_Product_Edit_Tab_Options_Option {
        ...
        if (!$this->_values && $optionsArr) {
            ...
            foreach ($optionsArr as $optionId=>$option) {
                ...
                if ($option->getIsDelete() != '1') {
                    ...
                    $value['price_curve_type'] = isset($optionsArr[$optionId]['price_curve_type'])?$optionsArr[$optionId]['price_curve_type']:1;
                    ...
                    $value['formula'] = $this->htmlEscape(isset($storeOptionsArr[$optionId]['formula'])?$storeOptionsArr[$optionId]['formula']:$option->getFormula());
                    $value['use_custom_title'] = $this->htmlEscape(isset($storeOptionsArr[$optionId]['use_custom_title'])?$storeOptionsArr[$optionId]['use_custom_title']:$option->getData('use_custom_title'));
                    $value['weight_per_one'] = $this->htmlEscape(isset($storeOptionsArr[$optionId]['weight_per_one'])?$storeOptionsArr[$optionId]['weight_per_one']:$option->getWeightPerOne());
                    $value['internal_note'] = $this->htmlEscape(isset($storeOptionsArr[$optionId]['internal_note'])?$storeOptionsArr[$optionId]['internal_note']:$option->getData('internal_note'));
                    ...
                    if ($optionModel->getGroupByType($option->getType())==Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT || $optionModel->getGroupByType($option->getType())==Magekart_CustomOptions_Model_Catalog_Product_Option::OPTION_GROUP_PRINTED) {                        
                        ...
                        if ($countValues>0) {
                            foreach ($option->getValues() as $key => $_value) {
                                ...
                                if ($_value->getIsDelete() != '1') {
                                    ...
                                    $value['optionValues'][$key] = $value['optionValues'][$key]+$_value->getData();
