...
class Magekart_Adminhtml_Block_System_Config_Form_Fieldset_Magekart_Support
	extends Magekart_Adminhtml_Block_System_Config_Form_Fieldset_Magekart_Abstract
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
		...
		$fields = array(
            ...
            array('type' => 'label', 'name' => 'send', 'after_element_html' => '<div class="right"><button type="button" class="scalable save" onclick="magekartSupport();">'.$this->__('Send').'</button></div><div class="notice" id="ajax-response"></div>'),
  		);
    ...
    protected function _getReasons()
    {
        ...
        $reasons[] = array('label'=>$this->__('Magento/PrintX Related Support (paid)'), 'value'=>'Magento v' . Mage::getVersion());
        $reasons[] = array('label'=>$this->__('Request New Extension Development (paid)'), 'value'=>'New Extension');
        foreach ($modules as $moduleName) {
            $name = explode('_', $moduleName, 2);
            if (!isset($name) || $name[0] != 'PrintX') {
                continue;
            }
    ...
    {
        $ajaxUrl = $this->getUrl('magekart/support');
        $html = parent::_getFooterHtml($element);
        $html = '<h4>'.$this->__('Contact PrintX Support Team or visit <a href="%s">%s</a> for additional information', 'http://help.printxinc.com/', 'help.printxinc.com').'</h4>' . $html;
