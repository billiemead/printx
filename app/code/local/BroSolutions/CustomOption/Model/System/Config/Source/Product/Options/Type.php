 ...
 */
class BroSolutions_CustomOption_Model_System_Config_Source_Product_Options_Type
{
    ...
    public function toOptionArray()
    {
        ...
        foreach (Mage::getConfig()->getNode(self::PRODUCT_OPTIONS_GROUPS_PATH)->children() as $group) {
            ...
            if($group->getName() == 'printing' && $product && $product->getTypeId() != 'printed')
                continue;
            $typesPath = self::PRODUCT_OPTIONS_GROUPS_PATH . '/' . $group->getName() . '/types';
            foreach (Mage::getConfig()->getNode($typesPath)->children() as $type) {
                if($group->getName() == 'printing' && !Mage::helper('brocustomoption')->checkType($type))
                    continue;
