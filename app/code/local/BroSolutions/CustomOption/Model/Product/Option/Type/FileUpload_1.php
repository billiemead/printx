...
class BroSolutions_CustomOption_Model_Product_Option_Type_FileUpload extends Magekart_CustomOptions_Model_Catalog_Product_Option_Type_Select
{
    /**
    ...
    public function getCustomizedView($optionInfo)
    {
        try {
            if (isset($optionInfo['option_value'])) {
                ...
                $html = '';
                $values = @unserialize($optionInfo['value']);
                if(is_numeric($values)){
                    $value = Mage::getModel('catalog/product_option_value')->load($values);
                    $value->addData(unserialize($value->getData('serialised_values')));
                    return $value->getSide();
                }
                foreach($values as $key => $value){
                    if($key == 'value_id')
                        continue;
                    $html .= $key.': '.$this->_getOptionHtml($value).'<br/>';
                }
                return $html;
            }
        } catch (Exception $e) {
    ...
    public function validateUserValue($values)
    {
        ...
        $valueId = $this->getUserValue();
        $value = $option->getValueById($valueId);
        if($value->getTitle() == 'later')
            return $this;
    ...
    public function getSufixBySelectedValue()
    {
        $option = $this->getOption();
        $valueId = $this->getUserValue();
        $value = $option->getValueById($valueId);
        ...
        return explode('_',$value->getSide());
    }
    ...
    protected function _validateUploadedFile()
    {
        ...
        $sufixes = $this->getSufixBySelectedValue();
        ...
        $file = $processingParams->getFilesPrefix() . 'options_' . $option->getId() . '_file_'.$sufix;
        try {
        ...
        // Image dimensions
        $_dimentions = array();
        if ($value->getImageSizeX() > 0) {
            $_dimentions['maxwidth'] = $value->getImageSizeX();
        }
        if ($value->getImageSizeY() > 0) {
            $_dimentions['maxheight'] = $value->getImageSizeY();
        }
        ...
        // File extension
        $_allowed = $this->_parseExtensionsString($value->getFileExtension());
        if ($_allowed !== null) {
        ...
        if ($upload->isUploaded($file) && $upload->isValid($file)) {
            ...
            $oldValue = $this->getUserValue();
            if(!isset($oldValue)){
                $oldValue = array();
            } elseif(is_numeric($oldValue)){
                $oldValue = array('value_id'=>$oldValue);
            }

            
            $oldValue[$sufix]=array(
                'type'          => $fileInfo['type'],
                'title'         => $fileInfo['name'],
    ...
    protected function _getValidatorErrors($errors, $fileInfo)
    {
        ...
        foreach ($errors as $errorCode) {
            ...
            {
                $result[] = Mage::helper('catalog')->__("Maximum allowed image size for '%s' is %sx%s px.", $option->getTitle(), $value->getImageSizeX(), $value->getImageSizeY());
